...
export default function AdminRekap({ showToast }) {
  ...
  const doUnlock = async (item) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/api/rekap-replacement/${item.id}`, {
        method:"PATCH",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ tgl_isi: item.tgl_isi||null, jam_cash_in: item.jam_cash_in||null, jam_cash_out: item.jam_cash_out||null, denom: item.denom||100_000 }),
      });
    ...
    <div>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={item.id} style={{
                    background: item.is_saved ? "rgba(74,222,128,0.025)" : i%2===0 ? "transparent" : "rgba(255,255,255,0.012)",
                  }}>
                    ...
                    <Td>
                      {isEditing ? (
                        <select value={ed.denom} onChange={e => setEditRow(r => ({...r,denom:Number(e.target.value)}))} style={s.inlineSelect}>
                    ...
                    <Td>
                      {isEditing ? (
                        <input type="date" value={ed.tgl_isi} onChange={e => setEditRow(r => ({...r,tgl_isi:e.target.value}))} style={s.inlineInput} />
                      ) : <span style={{ color: item.tgl_isi?"#e2e8f0":"#374151", fontSize:11 }}>{item.tgl_isi || "—"}</span>}
                    </Td>
                    <Td>
                      {isEditing ? (
                        <input type="time" value={ed.jam_cash_in} onChange={e => setEditRow(r => ({...r,jam_cash_in:e.target.value}))} style={{...s.inlineInput,width:80}} />
                      ) : <span style={{ color: item.jam_cash_in?"#60a5fa":"#374151", fontSize:11, fontFamily:"monospace" }}>{item.jam_cash_in||"—"}</span>}
                    </Td>
                    <Td>
                      {isEditing ? (
                        <input type="time" value={ed.jam_cash_out} onChange={e => setEditRow(r => ({...r,jam_cash_out:e.target.value}))} style={{...s.inlineInput,width:80}} />
                    ...
                    <Td>
                      {isEditing ? (
                        <div style={{ display:"flex", gap:4 }}>
                          <button onClick={saveEdit} disabled={busy} style={s.saveSmBtn}>✓ Save</button>
      ...
      {confirm?.type === "unlock" && (
        <ConfirmModal
          ...
          desc="Data akan bisa diedit kembali."
          danger={false}
...
function Td({ children, mono, bold, dim, accent, small, truncate }) {
  return (
    <td style={{
      ...
      overflow: truncate?"hidden":"visible", textOverflow: truncate?"ellipsis":"clip",
...
function RekapStatusBadge({ status }) {
  ...
  return <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:4, background:bg, color:c }}>{s2||"—"}</span>;
}
function TableSkeleton() {
  return <div style={{ padding:"20px 0" }}>{[...Array(5)].map((_,i) => <div key={i} style={{ height:36, background:`rgba(167,139,250,${0.02+i*0.005})`, borderRadius:4, marginBottom:4 }} />)}</div>;
}
...
const s = {
  ...
  tableWrap:   { overflowX:"auto", borderRadius:10, border:"1px solid rgba(167,139,250,0.08)", background:"rgba(5,11,24,0.6)" },
